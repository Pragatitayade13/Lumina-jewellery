...
if (!admin.apps.length) {
  try {
    ...
    console.error('Firebase Admin init error in orders API:', error);
  }
}
...
async function handler(req, res) {
  ...
  try {
    ...
    // Non-supervisors only see their own assigned deliveries
    const isSupervisor = ['superadmin', 'admin', 'manager', 'finance'].includes(role);
    if (!isSupervisor) {
      // Fetch either explicitly assigned, or packed (open for pickup)
